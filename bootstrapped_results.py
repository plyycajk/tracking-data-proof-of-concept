...
#creates dictionary of labels and values from the subjects for use in the dropdown menu later
options = [{'label':i,'value':i} for i in order]

#logo to display in top left hand corner
...
app.layout = dbc.Container(style={'backgroundColor': colors['header_background']}, children = [
  ...
  dbc.Row(      
        html.H3('Subject Tracking', style={'color':colors['header_text']}) #section header
        ),
  dbc.Row(     
        dcc.Dropdown(id='sub_select', options = options) #subject dropdown
  ),
  dbc.Row(
  ...
  dbc.Row(
    html.H3('Subject comparison for selected student', style={'margin-top':'20px','color':colors['header_text']})), #section header
  dbc.Row(
...
#callback section - populates box figures based on the subject selection in the dropdown
...
#subsets data into each exam type based on the subject selected from the dropdown
...
#adds box plots for each exam series - adds hover data info in customdata
...
#adds subject name and number of students to title of boxplot to help user     
...
#creates scatter for individual student based on hovering on a particular datapoint on the box plot - shows exam series performance for the student in that subject
@app.callback(
    Output('student_scatters','figure'),
    [Input('sub_select','value'),Input('box_fig','hoverData')]
)

#creates subset for hovered student for the selected student
def scatter_plot(value, hoverData):
  ...
  if hoverData:
    if hoverData['points'][0]['customdata'][0] in student_list_progress and value in order_Yell_diff: #If no baseline data for student, or they have not sat the selected subject then graph is blank (prevents errors when changing subject with student already hovered over)
      ...
      trace_color= [colour for colour in trace_subset.colour.loc[:]] #extracts color of plot
      ...
      max_result = tracking_subset.loc[:,"Score"].max() #finds maximum score achieved by stident across exam series
      ...
      #adds trace for student for that subject for mocks and GCSE
      ...
      #adds trace for baseline data only (not part of the exam series)
      ...
      #adds reference line for baseline target grade
      scatter_fig.add_hline(y=yellis_target, line_width=1.5, line_dash='dash', line_color=trace_color[0])
      
      #adds reference line above and below baseline data showing range of grades achieved across the series (and comparison versus baseline)
      ...
      scatter_fig = go.Figure(layout=scatter_layout) #blank figure returned from earlier IF statement
 
  return scatter_fig


#student comparison charts based on clicking a data point on the boxplot
@app.callback(
...
#subset of data for all of a given student's (on click) GCSE versus baseline for all subjects
  if clickData:
...
#adds traces for GCSE and Baseline data
